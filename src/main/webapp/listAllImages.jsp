        ...
        <c:when test="${status.first or status.index % 4 eq 0}">
            ...
            <div class="col-xs-2 col-xs-offset-2">
                <a href="#" class="thumbnail text-center">
                    <img name="${image.imageName}" date="<fmt:formatDate value='${image.uploadTime}' pattern='yyyy-MM-dd HH:mm'/>" style="width: 140px; height: 130px;" src="http://p53x2m122.bkt.clouddn.com/${image.url}"><!--这里请填写自己的链接地址，详见下图-->
                    <input class="pull-left" type="checkbox" value="${image.imageId}" url="${image.url}"/>${image.imageName }
                </a>
            </div>
        </c:when>
            ...
            <div class="modal-body">
                <form class="form-horizontal" role="form" id="form">
                    <div class="form-group"  >
                        <label for="imageName" class="col-xs-2 control-label" >Title</label>
                        <div class="col-xs-4">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#upload').click(function() {
            if ($('#imageName').val() == '' || $('#image').val() == '') {
            } else {
                $('#form').attr('action', '${pageContext.request.contextPath}' + '/uploadImage.html');
        ...
        //点击确定退出
        $('#exit').click(function() {
            $.get('${pageContext.request.contextPath}' + '/doLogout.html', function(data, status) {
        ...
        $('#delete').click(function() {
            ...
            $.post('${pageContext.request.contextPath}' + '/deleteImage.html', {
                ids: ids,
                urls: urls
            },function(data, status) {
                $('#myModa3').modal('hide');
                location.href = '${pageContext.request.contextPath}' + '/listAllImages.jsp';
            });
        });
    });
</script>
